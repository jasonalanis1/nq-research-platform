...
| M17 | Intraday periodicity: same half-hour slot, next day | Institutions executing on a fixed daily schedule (fund flows, VWAP/TWAP programs, end-of-day rebalances) trade the same direction in the same slot on consecutive days (Heston-Korajczyk-Sadka 2010 JF) | Sign-adjusted OPEN-slot and CLOSE-slot return on day t+1 given the same slot on day t; MIDDAY slot as the specificity control; first-minute concentration (kill) | Intraday (30-min hold, time-based) | NEW (September 12th, ~4:40 pm CT test cycle, literature channel, sourcing toward the shelf floor). Cross-day same-slot claim: not a resurrection of M16 (same-day first->last), M1/hyp-000110 (close -> next open), the IB family, or calendar effects. NULL 1 = each slot's own drift. 17 cells, 2 gating. Data on disk. Mechanism doc research/mechanisms/intraday-periodicity-same-slot-next-day-m17.md. DRAWABLE, map-ranked #3. | (Entry 21) |
