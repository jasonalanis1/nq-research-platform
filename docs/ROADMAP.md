...
**Current status: zero setups have cleared the promotion bar (seven strategy hypotheses tested -- six reversal-thesis, one continuation-thesis, all REJECTED -- plus two characterization studies finding no raw predictive relationship in either the open's own price direction or breakout volume). Three lines of evidence now argue that more intraday-price-action/volume patterns built the same way have a low expected payoff without new information. Candidates worth considering next: a time-of-day-controlled volume baseline (the one gap in today's volume test); day-of-week or calendar-position effects; or a genuinely nonlinear/conditional version of the persistence question already tested. Deciding among these is the next call to make.**
