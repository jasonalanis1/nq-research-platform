...
**Current status: zero setups have cleared the promotion bar (seven strategy hypotheses tested: close_any, close_min_distance, full_bar_range, the FVG entry trigger, the two trend-structure-liquidity-filter variants, and Initial Balance Breakout -- six reversal-thesis, one continuation-thesis, all REJECTED -- plus one characterization study finding no raw predictive relationship at any tested horizon). Two lines of evidence now argue that more intraday-price-action-only chart patterns have a low expected payoff without new information. Candidates worth considering next: whether the data's Volume column carries real, usable values (untested so far -- everything to date has used price only); day-of-week or calendar-position effects; or a genuinely nonlinear/conditional version of the persistence question just tested. Deciding among these is the next call to make.**
