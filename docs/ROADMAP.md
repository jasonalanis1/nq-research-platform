...
**2026-09-02 (same day) -- out-of-sample Validation check, first in this project's history.** Because exp-039 is the first result this project has ever considered acting on, and because 1 hit out of 14 hypotheses at a 90% CI is within the ~1.4 false positives expected by chance, both Claude and the Advisor recommended a Validation-slice replication check before any further Discovery-only testing (FOMC) or structure design; Jason approved it. This is a disclosed, one-time exception to `docs/RESEARCH_INTEGRITY_PROTOCOL.md`'s normal rule that Validation is reserved for a formally-promoted candidate -- this characterization study never went through that trade-count-based bar. The check reuses the frozen spec's functions completely unmodified, zero new fitting.

**Result: REPLICATED.** The first run found zero release days in the Validation window -- caught immediately as a coverage gap (the frozen calendar had never been extended past Discovery's end), not reported as a null. A fresh, independently-verified 27+27-date extension (2021-10-04 to 2024-01-03, same bls.gov sourcing and day-of-week/overlap verification discipline) was added, confirmed not to change the already-committed Discovery result at all, and the check re-run. Validation result: +101.925 pts mean |return| difference, 90% CI [+77.676, +128.011] -- entirely above zero. Both robustness checks and both CPI-only/NFP-only sub-groups replicate. The release/normal volatility ratio actually grew from 2.44x (Discovery) to 5.54x (Validation) -- a higher-volatility macro regime (the 2022 rate-hike cycle falls inside the Validation window) coinciding with a *stronger*, not weaker, relative effect, disclosed honestly as plausible context rather than an after-the-fact excuse. Full account: `research/experiments/exp-039-scheduled-macro-release-volatility.md`.

**Current status: zero setups have cleared the promotion bar, but for the first time this project has a result that survived an out-of-sample check.** Fourteen hypotheses tested across five mechanism families; thirteen null, one (exp-039, scheduled CPI/NFP release volatility) now confirmed on both Discovery and Validation. It remains a magnitude finding, not a directional one -- the natural next step is a volatility-capture design question, not a simple long/short rule, and this project's broker/execution planning has so far been built around directional futures positions, not options or volatility structures. Per the mandatory Advisor-consultation rule, both Claude's own read and the Advisor's fresh, independent read on what this replication changes and what to do next are being presented to Jason before any direction is proposed.

**2026-09-02 (same day) -- FOMC tested, second positive result.** After exp-039 replicated on Validation, both Claude and the Advisor recommended FOMC as the proportionate next step over structure design (premature -- no directional setup exists to attach a risk overlay to) or a second Validation look (reserved for now); Jason approved. Before sign-off, the Advisor reviewed the specific frozen spec draft (not just the direction decision, per the mandatory-consultation rule) and caught a real gap: the calendar-overlap check the feasibility report had flagged in advance was never actually run. Fixed before implementation: 6 FOMC/CPI same-day overlaps found and excluded from the primary classification (47 of 53 FOMC dates remain), 0 FOMC/NFP overlaps, and 14 of the 47 primary dates found to also fall inside an expiration week -- disclosed, not excluded, since exp-035 already found that variable null on its own. One new piece of machinery was needed and disclosed in advance: `compute_forward_return_at()`, since FOMC releases at 2:00 PM ET rather than the 8:30 AM anchor every other intraday study uses.

**Result: POSITIVE (exp-040) -- the second non-null result in this project's history.** Mean 30-minute post-2:00-PM |return| is +13.759 points larger on FOMC days than normal days, 90% bootstrap CI [+8.263, +19.831] -- entirely above zero, well past the 1.5-point threshold. Both robustness checks confirm; all four secondary horizons agree in direction. The release/normal ratio (2.22x) closely matches exp-039's own Discovery ratio (2.44x), strengthening the case that announcement-volatility clustering is a real, general mechanism rather than an artifact of how CPI/NFP happened to be tested. Same disclosed limitation as exp-039: a magnitude finding, not a directional one. Scoped to Discovery only; Validation replication is a separate future decision. No ledger entry. See `research/experiments/exp-040-fomc-release-volatility.md`.

**Current status: zero setups have cleared the promotion bar, across sixteen hypotheses tested spanning five mechanism families -- but the scheduled-information family now has TWO independently confirmed positive results (CPI/NFP, replicated on Discovery and Validation; and now FOMC, confirmed on Discovery) out of two tests.** Fourteen other hypotheses across four other families remain null. The scheduled-information mechanism looks like a real, general feature of this market -- not a fluke specific to one release type -- but both findings are still magnitude, not direction, and neither has anywhere to attach yet: no directional setup has ever cleared this project's promotion bar for a volatility-aware risk overlay to sit on top of, and no options/volatility-structure execution infrastructure exists. Per the mandatory Advisor-consultation rule, both Claude's own read and the Advisor's independent read on what two-for-two in this family changes are being presented to Jason before any direction is proposed.

**2026-09-02 (same day) -- options/volatility-structure execution feasibility checked.** With the scheduled-release-volatility family 2-for-2 (exp-039, exp-040), the Advisor recommended checking whether a volatility-capture structure is even executable before any design work -- the same discipline already applied to the ES data purchase and the Robinhood futures-API gap. Checked (docs/OPTIONS_VOLATILITY_EXECUTION_FEASIBILITY.md): real NQ futures options ARE programmatically reachable via Interactive Brokers' TWS API (confirmed FuturesOnOptions contract support, $0 account minimum, but a margin account and a discretionary futures-options approval gate are both required) -- but not through Tradovate (options viewing exists, no confirmed order-placement API) or NinjaTrader's Continuum connection (explicitly not supported for the automation stack already validated for directional futures). A cheaper fallback needs no options at all: a stop-width/position-sizing overlay on a plain directional futures position is already executable today on the exact Tradovate/NinjaTrader + CrossTrade/PickMyTrade stack this project's broker check already confirmed, via documented relative/dynamic bracket-order offsets. No broker account opened, no approval requested, no code written, no decision made.

**2026-09-03 -- historical options/implied-volatility data feasibility checked.** The Advisor's next recommendation after the options-execution feasibility check: before designing any volatility-capture options structure, check whether the DATA to price it honestly even exists -- the same discipline already applied to the ES purchase, the economic-calendar data, and options execution itself. Checked (docs/OPTIONS_DATA_FEASIBILITY.md): real NQ futures options chain data IS reachable through Databento (the vendor already used for this project's futures data), but only as raw trade/quote data -- Databento does not compute Greeks or implied volatility, so this route would require building options-pricing/IV-derivation machinery from scratch, a category of work this project has never done. Exact cost not yet known (usage-based pricing, needs a cost-calculator run against a specific range). CME Group sells a pre-computed Greeks/IV product that would skip that problem, but pricing requires a direct sales quote. A cheaper, free-or-near-free approximation exists (VXN or NDX/SPX index options data) but is a genuinely different underlying product (cash-settled index options vs. American-style futures options), not a substitute, and would need disclosure as such. No purchase made, no code written, no account opened, no experiment, no ledger entry.

**2026-09-03 -- options idea shelved as scope creep; next directional idea found, specified, and tested.** After both feasibility checks, Claude and the Advisor independently recommended shelving the options/volatility-structure idea: a new broker (IBKR) behind a discretionary approval gate, unbuilt options-pricing/IV-derivation machinery, and a data cost that wasn't even quoted yet, all in service of a structure with nowhere proven to attach it (no directional setup has ever cleared the promotion bar). Jason agreed and asked to find the next idea to test. A research pass proposed three candidates (turn-of-month seasonality, unconditional overnight drift, order-flow imbalance); the Advisor rejected all three (turn-of-month risks being a third null calendar-timing variant after day-of-week and expiration-week; order-flow repeats the exact scope-creep shape just shelved) and proposed a fourth instead: does the initial direction of NQ's move right after a CPI/NFP/FOMC release continue over the next few hours -- the natural follow-up question to exp-039/040's own confirmed-but-magnitude-only findings, using only machinery the project already has. Claude agreed it was the strongest option; Jason approved drafting a frozen spec.

**Frozen spec drafted, Advisor-reviewed, signed off, implemented (exp-041).** The Advisor's focused technical review of the draft caught one overclaimed disjointness statement and a resulting ambiguity about 6 overlap-day dates (fixed by having the new module reuse each source module's own classify_day() unmodified, in two separate passes, rather than write a new unified classifier), plus a real scoping gap in gate condition 5 (a pooled positive result could have been carried by CPI/NFP alone while FOMC disagreed, given a 3.4:1 weight imbalance) -- fixed with an explicit per-subtype sign-agreement disclosure requirement. Jason signed off; implemented with 16 new unit tests (176 total passing) and reused, unmodified, every piece of machinery from exp-039/040 -- zero new calendar data, zero new detection code.

**Result: NULL (exp-041) -- the fourteenth null hypothesis of seventeen tested.** Mean directional continuation (pooled cpi+nfp+fomc) -2.947 pts, 90% CI [-11.328, +4.361] -- spans zero, point estimate negative. Both robustness checks stay null; the effect's sign is not even stable across a first-half/second-half split. One disclosed, unplanned side-observation: the cpi-only descriptive breakdown shows a statistically credible reversal tendency (CI entirely below zero) -- recorded honestly as a lead, not promoted, since it wasn't pre-registered and this project's rules require a hypothesis frozen before the data that tests it, not fitted after. Does not undermine exp-039/040's own confirmed magnitude findings. No ledger entry. See `research/experiments/exp-041-post-release-directional-continuation.md`.

**Current status: zero setups have cleared the promotion bar, across seventeen hypotheses tested spanning five mechanism families.** The scheduled-information family remains the project's strongest lead (two confirmed magnitude results, CPI/NFP and FOMC) but has now also produced its first directional null: the extra volatility around scheduled releases does not, pooled across release types, resolve into a predictable continuation direction. The disclosed cpi-only reversal observation is a real but unconfirmed lead, not yet a tested hypothesis. Per the mandatory Advisor-consultation rule, both Claude's own read and the Advisor's independent read on what this null means for what's next are being presented to Jason before any direction is proposed.

**2026-09-03 (later the same night) -- the missing DSR/PBO tool built, fixed, and applied for real, per the Advisor's recommended priority.** After exp-041's null, Jason asked both "where's the stopping point" and "find other strategies." The Advisor's independent read (per the mandatory-consultation rule) recommended a specific order: finish and apply the already-sketched-but-never-really-used `larry_validate.py` DSR/PBO tool BEFORE hunting hypothesis #18 or building the stop-width overlay, since seventeen hypotheses have now been searched by hand with no formal multiple-testing correction ever actually applied to a real result -- exactly the failure mode DSR/PBO exists to catch. Jason agreed: "Do what you have to do first."

**Installed and re-verified `purgedcv` for real.** Despite being cited as available in ~15+ prior docs, the library had never actually been installed in this environment -- every prior mention was trusting an old note, not a live check. Installed it and re-ran the synthetic sanity check (50 trial variants, 49 noise + 1 small planted edge): picked a noise config as "best," as expected; DSR correctly deflated the picked config from 0.988 (naive, ignoring the 50 trials) to 0.548-0.568 (honest); PBO came in at 0.42-0.61 across runs, correctly flagging the pick as overfit. Confirmed a real, working library, not a stub.

**Found and fixed a real, silent bug in `larry_validate.py`'s trial-counting.** The original sketch counted trials by matching `strategy_name` in the ledger -- but every real hypothesis in this project has a distinct strategy_name, even genuine side-by-side variants, so this always returned n_trials=1, silently disabling DSR's entire multiple-testing correction. This was worse than the module's own self-disclosed "design question to sharpen later" note implied -- it was a functional no-op, not an unrefined feature. Fixed by walking `parent_hypothesis_id` lineage to group hypotheses sharing a root ancestor (a real improvement, verified against the real ledger: correctly groups hyp-000001 with hyp-000007, hyp-000003 with hyp-000008), plus an explicit `n_trials_override` parameter for the case lineage-walking still can't capture -- a joint search producing siblings under different immediate parents. 12 new unit tests (`tests/test_larry_validate.py`, 188 total in the suite now).

**First real application: Level Sweep Reversal liquidity-filter family (hyp-000007, hyp-000008).** Re-ran `src/_run_liquidity_filter_discovery_backtest.py` against Discovery data, confirming it reproduces exp-026/027's originally published trade counts exactly (44 resolved protected close_min_distance trades, 59 resolved protected full_bar_range trades) before trusting the output. Built a daily-aligned return series over all 2,101 real Discovery trading days for each of the 4 variant configs actually searched together (close_min_distance and full_bar_range, protected and not-protected), satisfying `probability_of_backtest_overfitting()`'s shared-time-axis requirement. Evaluated hyp-000007 and hyp-000008 with `n_trials_override=4` (documented reasoning: all 4 configs were genuinely compared together in one real script run, which lineage-walking alone can't see since the two hypotheses have different immediate parents).

**Result: both REJECTED by DSR/PBO.** hyp-000007: DSR 0.366 (well below the 0.90 threshold). hyp-000008: DSR 0.068. Both PBO 0.278 (above the 0.25 threshold too -- fails on both counts, not just one). Consistent with these candidates never having cleared this project's own promotion bar in the first place (see exp-026/027) -- DSR/PBO now gives that informal rejection a real, trial-adjusted statistical basis instead of relying on the promotion bar alone. Logged into the research ledger as new, non-destructive append-only status updates (the ledger's existing history is preserved, nothing was edited or deleted).

**Search-stopping-point checkpoint policy written down** (per the Advisor's recommendation, adopted as-is): review every 5 newly-tested hypotheses whether genuinely distinct mechanism families are still being found versus thin variants of families already rejected, logged explicitly rather than felt. Added to `docs/RESEARCH_INTEGRITY_PROTOCOL.md`. This checkpoint is retroactively due now (17 hypotheses tested, no review has ever been logged) -- next hypothesis-hunting session should open with it.

**A real, disclosed schema gap logged, not fixed tonight**: `research_ledger.py` has no field recording "these hypotheses were tested together in one batch," which is the actual limit on how precisely trial-counting can be automated. Logged in `docs/BACKLOG.md` as a `search_batch_id` idea for later, kept out of tonight's scope.

**2026-09-03 (later still) -- options route actually priced out, per Jason's "less is more, give me both ideas" step-back decision to try option one first.** Ran the real numbers instead of guessing: Databento prices historical options data by volume (GB) not by strike count, with $125 in free signup credit, and the actual need is narrow (a few hours on ~200 known event days, not continuous multi-year data) -- likely modest dollar cost, not yet exactly confirmed (needs their cost calculator, a free step). The earlier "build options-pricing math from scratch" engineering risk turns out to be smaller too: QuantLib (pip-installable, actively maintained) implements Black-76 and American exercise out of the box -- same shape as this project's own purgedcv precedent. Rough estimate: 3-5 focused sessions, not the vague "big lift" it was shelved as.

**But the Advisor's review of that estimate surfaced a bigger, cheaper, more important question than either cost line: has anyone checked whether this project's own confirmed findings (bigger price moves around CPI/NFP/FOMC) would have actually been tradeable through options, or does the options market already price that in ahead of time (the volatility risk premium)?** That check has never been done, needs no new engineering, and is free (VXN data, already identified as likely-free in `docs/OPTIONS_DATA_FEASIBILITY.md`). Full writeup: `docs/OPTIONS_STRUCTURE_COST_ESTIMATE.md`. Per the mandatory Advisor-consultation rule, both takes were shown to Jason before any next step was proposed; the free IV-vs-realized check is the recommended next move, before spending anything on Databento or QuantLib.

**2026-09-03 (later still) -- the free volatility-risk-premium check, done via literature rather than this project's own data (device link down).** Jason asked to push forward as far as possible without computer access. Found real, sourced academic evidence directly on point: NBER Working Paper 28306 ("Event-Day Options") studied S&P 500 and Treasury futures options around FOMC and employment-report days specifically, and found implied volatility has historically OVER-predicted what actually happened on those days -- a positive variance risk premium (0.248 percentage points on FOMC days, 0.204 on NFP days, for S&P 500). CBOE's own research desk independently corroborates the FOMC mechanism. CPI specifically was not found directly studied -- flagged as a real gap, not glossed over, though it's the same general category of event and the variance risk premium is broadly documented across most option markets.

**Also corrected a small inaccuracy in `docs/OPTIONS_DATA_FEASIBILITY.md`:** VXNCLS (the Nasdaq-100 volatility index) IS available for free on FRED itself, back to 2001-02-02 -- the earlier finding that it was "absent from FRED" only checked FRED's own listing page and missed the actual series. Full writeup: `docs/OPTIONS_VOLATILITY_RISK_PREMIUM_CHECK.md`.

**The Advisor's review of this literature check (fresh instance, read via a local clone since the device link was still down)**: the variance risk premium is a broad, well-replicated pattern across equity index options generally, so it's fair to treat this as "the base rate is against the idea," not proof it's dead -- NQ/Nasdaq-100 options could differ in magnitude from the S&P 500 proxy studied. Recommended: don't drop the idea, don't spend anything more on it either -- the free VXN-vs-this-project's-own-209-event-days comparison remains the right next step, now sharpened by having a real prior to test against. Also raised a fair critique of the literature pass itself: it was launched to test a cautionary hypothesis and stopped once it found supporting evidence, without a matching search for disconfirming research -- a one-sided search, disclosed here rather than hidden. Claude's own read agreed with most of this but pushed back on one point: the Advisor suggested even a favorable result "unlocks nothing" since no directional setup has ever cleared the promotion bar -- but the options-structure idea is explicitly non-directional by design (that was the reason for looking at it), so it doesn't need a directional setup to attach to the way the earlier stop-width overlay idea did. Both takes shown to Jason before any next step, per the mandatory Advisor-consultation rule; net result is unchanged from before this check -- still waiting on this project's own real data before deciding either way.

**2026-09-03 (later still) -- the project's own real VXN check, once the device reconnected.** Jason downloaded the real VXN (Nasdaq-100 volatility index) history directly from FRED after two attempts (the first covered only 2021-2026; the corrected `VXNCLS_MAX.csv` covers 2001-2026, comfortably spanning Discovery). Built `src/study_vxn_pricing_check.py`: compares VXN levels the day before each of this project's own exact CPI/NFP/FOMC Discovery event days (the same 160 CPI+NFP and 47 FOMC days already frozen and used by exp-039/040) against normal days.

**Result: honestly inconclusive, not a confirmation or a kill.** Test 1 (VXN the day before an event vs. a normal day) found no statistically significant difference for CPI, NFP, or FOMC -- VXN just doesn't move measurably ahead of these specific known dates. Test 2 (a rougher implied-vs-realized comparison) looked informative at first -- implied over-predicted realized moves by about +9.8 points on CPI+NFP event days -- until a baseline control (running the identical comparison on ordinary days) showed an even BIGGER gap on ordinary days (+21.3 points) than on event days. That means the apparent "over-prediction" was a property of the modeling assumption used to convert VXN into a 30-minute figure, not evidence about event-day pricing specifically -- caught before it could be oversold, and reported as uninformative rather than kept as a false positive.

**Why the null result doesn't settle anything either way:** VXN is a smoothed 30-day blended volatility average. One specific day's event contributes roughly 1/30th of that average, so even a real, meaningful one-day IV spike would be nearly invisible at the VXN level -- this project's own check used the wrong instrument to see the sharp version of this question, not evidence that no such effect exists. The real published academic evidence found earlier this same day (NBER WP 28306, using actual short-dated event options rather than a blended index) remains the best evidence this project has. Full writeup: `docs/OPTIONS_VOLATILITY_RISK_PREMIUM_CHECK.md`. Also corrected `docs/OPTIONS_DATA_FEASIBILITY.md`'s VXN-availability claim in place (it is free on FRED, contrary to that document's original finding).

**The Advisor's review of this real check (fresh instance, re-ran the script itself rather than trusting the doc's numbers)**: confirmed the methodology is sound and the baseline-control catch was real discipline, not spin -- but pointed out plainly that the VXN-is-too-blunt-an-instrument conclusion is a five-minute back-of-envelope argument (a single day is ~1/30th of a 30-day average) that didn't strictly require running code to anticipate, even though confirming it empirically was still the right call. Recommendation: get the real Databento options-data cost-calculator number next (15 minutes, no cost, still not done after being flagged twice the same day) before committing to the full multi-session QuantLib build, and do the overdue mechanism-family checkpoint review (17 hypotheses, due at 15, never logged) alongside it, since four of five mechanism families are dead nulls and this scheduled-information thread is the only one still alive. Both takes shown to Jason before any next step, per the mandatory Advisor-consultation rule.

**2026-09-03 (evening) -- stepping back, and closing out the CPI reversal lead.** After the real VXN check came back inconclusive, Claude laid out the full picture honestly: 17 hypotheses, zero promoted, the one live thread (scheduled-macro-release) two-for-two on "inconclusive, would need real money/data to know for sure." Jason said "Let's step back." A fresh Path-to-Profitability Advisor consultation, focused on this exact question, agreed 17-for-0 is not itself a bad sign (a normal rate for honest research on a heavily-traded instrument) but recommended pausing before spending more on the options thread specifically, given its risk profile (the real evidence points toward selling volatility, not a clean low-risk buy).

Jason then asked, in plain terms: is NQ futures "washed out," should we consider individual stocks, and what's actually most likely to work -- explicitly asking Claude to research this and give a real recommendation, and to explain results more simply going forward (saved to memory). A second Advisor consultation, focused on this question, plus Claude's own review of the full experiment history, agreed on: NQ isn't washed out (most of what's been tried is one *style* -- same-morning price-pattern reading -- not the full space), individual stocks should be parked (multiplies the number of things searched, which multiplies the odds of finding a fake edge by chance, and throws away everything already learned about NQ), and the two cheap, concrete next steps are the overdue checkpoint review and closing out exp-041's disclosed CPI-only reversal side-observation.

Jason agreed to both, explicitly not wanting to "rabbit hole" the CPI-reversal check, so it was scoped narrowly: reuse exp-041's own statistic unmodified, add only real trading costs (no new stop/target parameters invented), explicitly disclosed as not independent confirmation since it's the same Discovery data already looked at. Result (exp-042): a close-but-real kill -- positive point estimate, economically meaningful, but the 90% CI no longer excludes zero once the standard round-trip cost is applied (CI [-0.314, +21.810], versus [+0.436, +22.560] before cost). Logged as the closest near-miss the scheduled-information family has produced, closed out rather than tuned further, per Jason's explicit direction.

The checkpoint review itself (retroactively due since hypothesis 15, now at 18) was completed the same day: four of five mechanism families dead, the fifth alive only on magnitude, not direction. Go/no-go decision, made with Jason: pause deepening the options thread, don't move to stocks, explore order-flow/buying-vs-selling-pressure signals as the one genuinely untried style within NQ -- pending a real data cost check (Databento sells this as CME MBO/MBP-10 data; no exact price found yet, same "get the real calculator number" next step as the options thread). Full reasoning: `docs/RESEARCH_INTEGRITY_PROTOCOL.md`'s checkpoint-policy section, `docs/BACKLOG.md`'s Active/Parked entries, `research/studies/cpi-reversal-followup.md`, `research/experiments/_index.md`'s exp-042 entry.

**2026-09-03 (later still) -- order-flow priced out for real, and parked; three checks run in parallel per Jason's request.** The order-flow/buying-vs-selling-pressure candidate from the checkpoint review above was actually priced, live, via Databento's own interactive pricing calculator (accessed with the built-in browser, no account needed): the per-GB rate itself is affordable (MBO $1.80/GB, MBP-10 $0.50/GB), but full historical depth access (the years of history this project's Discovery/Validation split needs) is capped at the last 1 month on both the free and $199/month plans -- the Unlimited plan required for real history costs $4,500/month on an annual contract (~$54,000/year), a decisive blocker independent of the per-GB rate. Parked in `docs/BACKLOG.md`, not pursued further.

Jason, worried about running out of options, asked what else the Advisor could offer, then asked to write several small checks in parallel: a CFTC Commitment-of-Traders positioning check, a turn-of-month side-check, and a cheap confirmation of the Tradovate/NinjaTrader broker research (explicitly framed by Jason as possibly a new lead; the Advisor's independent take, obtained per the standing mandatory-consultation rule, was clear that broker research is not a new hypothesis and is premature relative to where the hypothesis search stands, but is cheap insurance worth doing in parallel).

Direct network access to cftc.gov turned out to be blocked from both this project's cloud sandbox and the local device (an organization egress restriction, confirmed via the proxy's own status endpoint -- not a data problem). Real CFTC data was still sourced, via a GitHub mirror of the CFTC's own published historical files, covering 2015-2018 (a real, disclosed limitation -- short of the full 2015-2021 Discovery window, but entirely inside it, zero leakage risk). Two frozen specs were written and run same-day: exp-043 (turn-of-month) and exp-044 (COT positioning), both structured as cheap, two-step-gated Step-1-only checks per the Advisor's guidance, and both using a drift-controlled difference-of-means design -- the same control this project's own VXN check needed earlier the same day, after skipping it produced a false positive there. Both came back clean, honest nulls: turn-of-month CI [-2.90, +17.44] (spans zero, while the non-turn-of-month baseline is itself significant -- i.e. ordinary NQ drift, not a turn-of-month effect); COT positioning CI [-39.29, +19.09] (wide, spans zero by a large margin). See `research/experiments/_index.md`'s exp-043/exp-044 entries, `research/studies/turn-of-month-check.md`, and `research/studies/cot-positioning-check.md`.

The broker confirmation was also completed: Tradovate and NinjaTrader both confirmed to support NQ/MNQ directly, with a mature, multi-vendor ecosystem of TradingView-webhook automation products (CrossTrade, PickMyTrade, QuantOTC, ClearEdge, flowbots.ninja) beyond the two already on file -- logged as an addendum to `docs/EXECUTION_BROKER_FEASIBILITY.md`, explicitly scoped as confirmation of an existing open question, not a new lead, per the Advisor's framing.

**2026-09-03 (later still) -- promotion-bar review, a genuine ensemble-thinking discussion, and the scheduled-macro-release thread closed for good.** Jason asked for a plain-language walkthrough of exactly how "winner" and "loser" get decided (the promotion bar, the Discovery/Validation/Holdout structure, the multiple-testing correction) -- confirmed he understood it correctly, nothing changed. He then asked whether the project is holding its own bar too strictly, given twenty straight non-promotions, and specifically whether "combining many smaller real things" (rather than searching for one big edge) is a legitimate direction -- citing the intuition that a firm doesn't need one big edge if it can find something that works a majority of the time. Real research confirmed this is a genuine, mainstream idea in professional quant trading (Grinold's "Fundamental Law of Active Management": combined signal quality scales with both the strength AND the number of roughly-independent signals). Two Advisor consultations the same day both flagged the same honest catch: this project currently has too few genuinely non-zero findings (essentially one -- the CPI/NFP/FOMC magnitude effect -- joined at the hip with one near-miss) for the "many weak signals" trick to do real work yet, and the disciplined version of "combine what we've learned" is a single, pre-registered multi-factor model built from features already computed across past studies -- not informal pattern-hunting across 20 old write-ups, which would reintroduce exactly the data-dredging risk the project's whole Discovery/Validation/DSR-PBO structure exists to prevent. Also surfaced: this project's own `docs/RESEARCH_INTEGRITY_PROTOCOL.md`, written back on 2026-08-20, already planned for exactly this -- a future "Strategy R&D Agent" (Phase 2: generate/mutate/combine) gated behind the chronological data split and Larry's DSR/PBO tooling, both of which are now confirmed actually built.

One concrete, cheap next step came out of this discussion and was run the same day: exp-045, pooling CPI+NFP for the reversal bet exp-042 tested CPI-only (a widening that was part of exp-041's original design, not a reach for a better number). Disclosed in advance: exp-041's own data already showed CPI-only and NFP-only pointing in opposite directions, so this could dilute the near-miss rather than rescue it -- and that is exactly what happened. Pooled n=152 (the first time this lead ever reached the 150-trade minimum), net reversal mean +3.775 pts, 90% CI [-4.963, +12.939] -- clearly spans zero. The scheduled-macro-release DIRECTION thread (exp-041/042/045) is now closed out for good; only the magnitude-only finding (exp-039/040, not itself tradeable) survives from that family. See `research/experiments/_index.md`'s exp-045 entry and `research/studies/cpi-nfp-pooled-reversal-followup.md`.

A real, frozen scoping plan for the multi-factor model was drafted in parallel -- see `research/studies/multi-factor-combination-scoping.md` -- for Jason and the Advisor to review before any of it is built, per the same line-by-line sign-off discipline used for the volatility-regime study.
