...
**Current status: zero setups have cleared the promotion bar (seven hypotheses tested now: close_any, close_min_distance, full_bar_range, the FVG entry trigger, the two trend-structure-liquidity-filter variants, and Initial Balance Breakout -- six reversal-thesis, one continuation-thesis, all REJECTED). Deciding what to test next.**
