...
JUDGMENT POINT (s.2/s.6): 40 trades or 6 weeks from the strategy's first
scored session, whichever comes first. KILL also on < 15 trades in 6 weeks.
...
from strategy_registry import PLUMBING_LOG_NAMES, read_rows as read_registry, in_paper as registry_in_paper  # noqa: E402
...
def measure(trades: list[dict], first_session: str | None, today: date | None = None) -> dict:
    ...
    if first_session:
        try:
            ...
            days = None
    weeks_left = None if days is None else max(0.0, JUDGE_WEEKS - days / 7.0)
    at_point = (n >= JUDGE_TRADES) or (days is not None and days >= JUDGE_WEEKS * 7)
    reason = None
    if n >= JUDGE_TRADES:
        reason = f"{JUDGE_TRADES} trades reached"
    elif days is not None and days >= JUDGE_WEEKS * 7:
        reason = f"{JUDGE_WEEKS} weeks elapsed" + (f" with only {n} trades (< {MIN_TRADES_AT_6_WEEKS}: KILL per s.6)" if n < MIN_TRADES_AT_6_WEEKS else "")
    return {
        "trades": n, "first_session": first_session, "days_elapsed": days,
        "trades_to_judgment": max(0, JUDGE_TRADES - n), "weeks_to_judgment": None if weeks_left is None else round(weeks_left, 1),
...
def book(log_rows: list[dict] | None = None, registry_rows: list[dict] | None = None,
    ...
    for reg in registry_in_paper(registry_rows):
        ...
        m = measure(tr, _first_session(log_rows, name) if name else None, today)
        ...
        if m["days_elapsed"] is None and entered:
            try:
                ...
                m["at_judgment_point"] = m["at_judgment_point"] or m["days_elapsed"] >= JUDGE_WEEKS * 7
            except ValueError:
                pass
...
def plain_lines(bk: dict) -> list[str]:
    ...
    for s in bk["strategies"]:
        head = f"- **{s['strategy_id']} {s['name']}** -- stage {s['stage']}"
        if s["trades"] == 0:
            days = s.get("days_elapsed")
            lines.append(head + f": in paper {days if days is not None else '?'} day(s), **0 trades scored yet** "
                                f"({s['sessions_scored']} session(s) checked). Judgment at {JUDGE_TRADES} trades or {JUDGE_WEEKS} weeks"
                                + (f" ({s['weeks_to_judgment']} weeks left)." if s.get('weeks_to_judgment') is not None else "."))
            continue
        wr = f"{s['win_rate']*100:.0f}%" if s["win_rate"] is not None else "n/a"
        avg = f"{s['avg_r']:+.2f}R" if s["avg_r"] is not None else "n/a"
        lines.append(head + f": **{s['trades']} trades** over {s['days_elapsed']} days; "
                            f"{s['trades_to_judgment']} trades or {s['weeks_to_judgment']} weeks to judgment"
