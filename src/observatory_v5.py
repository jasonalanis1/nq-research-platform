...
NQ Market Behavior Observatory, v5 -- frozen spec:
research/studies/observatory-v5-design-spec.md.

New event family: overnight session direction (16:00 close -> 09:30
open, signed) vs RTH forward returns from the open -- an occurrence
event distinct from v1/v2's level touches and v3's gap-magnitude/
volume-spike occurrences. Same baseline/ranking machinery as v1/v4,
including v4's ATR-normalized cost-aware screen.

Non-trading, exploratory ONLY. Discovery data only.

HOW TO RUN:
    python3 src/observatory_v5.py
"""
...
EVENT_TYPES = ["overnight_up", "overnight_down"]
N_BOOTSTRAP = 3000
RANDOM_SEED = 7
...
def main():
    print("=" * 78)
    print("NQ MARKET BEHAVIOR OBSERVATORY -- v5 (overnight direction, exploratory)")
    print("=" * 78)
    print("\nFrozen spec: research/studies/observatory-v5-design-spec.md\n")

    df, is_synthetic = load_price_data(context="observatory_v5.py")
    if is_synthetic:
    ...
    for day_num, day in enumerate(days_sorted):
        ...
        event_type = "overnight_up" if overnight_return > 0 else ("overnight_down" if overnight_return < 0 else None)
        ...
        # single event per day, measured at the open bar (i=0)
        bucket0 = time_bucket_for(times[0])
        m0 = measure(0)
        if bucket0 is not None and m0 is not None:
            n_events_total += 1
            for h, ret in m0.items():
                add(event_measurements, (event_type, regime, bucket0, h), ret)

        # baseline: every RTH bar (i>0) with full lookahead, sampled every 5th
        for i in range(1, n - max_h, 5):
    ...
    out = {
        "spec": "research/studies/observatory-v5-design-spec.md",
    ...
    out_path = DATA_DIR / "observatory_v5_results.json"
