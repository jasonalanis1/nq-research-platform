...
Six sections, always in this order, because that is the order an owner asks:
  ...
  2. THE PRODUCT  -- can the bot actually trade, what blocks the next step
  3. PIPELINE     -- what's in the funnel, what advanced, what died
  4. OPERATIONS   -- did the staff do their jobs, did the machinery hold
  5. SPEND        -- what it cost, against the caps
  6. YOUR DESK    -- what needs Jason, and nothing else
...
def product() -> list[str]:
    ...
    lines.append(f"- Milestones complete: **{len(done)} of {len(done) + len(missing)}**"
...
def pipeline(closed_today: int | None = None) -> list[str]:
    ...
    lines = ["**3. PIPELINE**"]
    lines.append(f"- Ideas on the shelf: **{shelf.split(' DRAWABLE')[0].replace('Shelf ', '')}** ready to test")
    lines.append(f"- Candidates in flight: **{n_open}**")
...
def build(label: str, moved: str = "", agents: str = "") -> str:
    ...
    for section in (money(), product(), pipeline(), operations(), spend(), your_desk()):
        out += section + [""]
