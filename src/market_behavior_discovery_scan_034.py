"""
market_behavior_discovery_scan_034.py
========================================

Scan 034. Draws Entry 27 (map anchor M23): Tokyo FX session-open
volatility burst, 6E (open scope -- signal and trade both in 6E).
Mechanism doc written BEFORE this scan:
research/mechanisms/tokyo-open-volatility-6e-m23.md.
...
FOUR pre-registered cells (scan_034_2026-09-13), 1 gating:
  1. Tokyo-open hour (03:00-04:00 ET) |return|/ATR14 mean MINUS
     unconditional RTH-equivalent-hour |return|/ATR14 mean, block CI  <== GATING (P1)
  2. pre-Tokyo hour (02:00-03:00 ET) |return|/ATR14 mean MINUS
  ...
  4. share of the Tokyo-open hour's |return| in the first minute
     (03:00-03:01 ET) (KILL)
...
HOW TO RUN:
    python3 src/market_behavior_discovery_scan_034.py
"""
...
def main():
    print("=" * 78); print("MARKET BEHAVIOR DISCOVERY ENGINE -- Scan 034 (Entry 31 / map M27: Tokyo-open volatility, 6E (companion to M23, non-overlapping window))"); print("=" * 78)
    fx_all, synth = load_price_data(context="market_behavior_discovery_scan_034.py", symbol="6E")
    if synth:
    ...
    tokyo_abs, pretokyo_abs, unconditional_abs, kill_rows = [], [], [], []
    ...
    for day, g in disc.groupby(disc.index.date):
        ...
        if atr is None or np.isnan(atr) or len(rth_equiv) < 200:
            continue
        tokyo = g.between_time("19:00", "20:00", inclusive="left")
        pretokyo = g.between_time("18:00", "19:00", inclusive="left")
        first_min = g.between_time("19:00", "19:01", inclusive="left")
        if tokyo.empty or pretokyo.empty or first_min.empty:
            days_skipped += 1; continue
        days_used += 1
        unconditional_abs.extend(hourly_abs_returns(rth_equiv, atr))
        lon_ret = (float(tokyo["Close"].iloc[-1]) - float(tokyo["Open"].iloc[0])) / atr
        pre_ret = (float(pretokyo["Close"].iloc[-1]) - float(pretokyo["Open"].iloc[0])) / atr
        fm_ret = (float(first_min["Close"].iloc[-1]) - float(first_min["Open"].iloc[0])) / atr
        tokyo_abs.append(abs(lon_ret)); pretokyo_abs.append(abs(pre_ret))
        kill_rows.append({"lon_abs": abs(lon_ret), "fm_abs": abs(fm_ret)})

    print(f"Discovery days usable (Tokyo hour + pre-Tokyo hour + first-minute bars present): {days_used}; skipped: {days_skipped}")

    lon_arr = np.array(tokyo_abs); pre_arr = np.array(pretokyo_abs); uncond_arr = np.array(unconditional_abs)
    ...
    cells = {
        "1_tokyo_minus_unconditional_GATING": {"tokyo_mean": float(lon_arr.mean()), "unconditional_mean": float(uncond_arr.mean()),
                                                  "diff": lon_diff, "ci_90": lon_ci, "credible_positive": bool(p1_credible),
                                                  "n_days": days_used, "n_unconditional_hours": int(len(uncond_arr))},
        "2_pretokyo_minus_unconditional_SECONDARY_UNPINNED": {"pretokyo_mean": float(pre_arr.mean()), "unconditional_mean": float(uncond_arr.mean()),
    ...
    print("\nCells:")
    c1 = cells["1_tokyo_minus_unconditional_GATING"]
    print(f"  1_GATING  tokyo_mean={c1['tokyo_mean']:.4f} unconditional_mean={c1['unconditional_mean']:.4f} "
          f"diff={c1['diff']:+.4f} ci_90=({c1['ci_90'][0]:+.4f},{c1['ci_90'][1]:+.4f}) credible_positive={c1['credible_positive']} n_days={c1['n_days']}")
    c2 = cells["2_pretokyo_minus_unconditional_SECONDARY_UNPINNED"]
    print(f"  2_SECONDARY pretokyo_mean={c2['pretokyo_mean']:.4f} unconditional_mean={c2['unconditional_mean']:.4f} "
    ...
    if k4["kill"] and c1["credible_positive"]:
        verdict, note = "KILLED", "Tokyo-open-hour response is concentrated in the 19:00 first-minute print -- a stale-quote/thin-liquidity artifact, not a genuine hour-long liquidity effect. Closes per kill rule, even though magnitude alone was confirmed."
    elif c1["credible_positive"]:
        if c2["credible_positive"]:
            verdict, note = "P1_PASS_AMBIGUOUS", "Tokyo-open hour credibly exceeds the unconditional baseline, but so does the pre-Tokyo hour -- suggests broader overnight-session elevation (overnight-coil territory) rather than a genuine session-boundary discontinuity specific to the Tokyo open. Advance to Statistical with this caveat disclosed, not a clean pass."
        else:
            verdict, note = "P1_PASS", "Tokyo-open hour credibly exceeds the unconditional hourly baseline and the pre-Tokyo hour does not -- concentrated at the session boundary as the mechanism predicts, and not a pure first-minute print artifact. Advance to Statistical per pipeline order."
    else:
        verdict, note = "P1_FAIL", "Tokyo-open hour |return| not credibly larger than the unconditional hourly baseline. Entry closes on falsifier (a)."
    print(f"\nVERDICT: {verdict}\n  {note}")

    out = {"n_days_used": days_used, "n_days_skipped": days_skipped, "cells": cells, "verdict": verdict, "note": note}
    (DATA_DIR / "market_behavior_discovery_scan_034_results.json").write_text(json.dumps(out, indent=2, default=float))
    print(f"\nWritten: {DATA_DIR / 'market_behavior_discovery_scan_034_results.json'}")
