"""
study_overnight_range_mid_10d_drift_h116_prospective.py
===========================================================

H116 -- frozen spec:
research/studies/overnight-range-mid-10d-drift-h116-prospective-spec.md.
Single pre-registered Validation-slice prospective test of H116
(overnight_range_vs_atr mid tercile -> positive 10d forward drift,
long, time-based exit). Unmodified thresholds and bucket edges from
the Discovery-slice pass -- nothing refit here.

HOW TO RUN:
    python3 src/study_overnight_range_mid_10d_drift_h116_prospective.py
"""
...
from study_overnight_range_mid_10d_drift_h116 import (
    VAR, BUCKET, BUCKET_LABELS, HORIZON_DAYS, MIN_ECONOMIC_R,
...
def main():
    print("=" * 78)
    print("H116 PROSPECTIVE: OVERNIGHT-RANGE-MID 10-DAY DRIFT, VALIDATION SLICE")
    print("=" * 78)
    print("\nFrozen spec: research/studies/overnight-range-mid-10d-drift-h116-prospective-spec.md\n")

    df, is_synthetic = load_price_data(context="study_overnight_range_mid_10d_drift_h116_prospective.py")
    if is_synthetic:
        print("ABORT: only synthetic data available.")
        return

    # Bucket edges frozen on the FULL DISCOVERY sample -- identical to the
    # Discovery-slice script. Never recomputed on Validation data.
    ...
    discovery_result = {
        "n": 435, "mean_r_multiple_net": 0.6822591279531698,
        "ci_90": [0.5062488020995606, 0.8536106460992385],
    }
    ...
    out = {
        "spec": "research/studies/overnight-range-mid-10d-drift-h116-prospective-spec.md",
    ...
    out_path = DATA_DIR / "study_overnight_range_mid_10d_drift_h116_prospective_results.json"
    ...
    notes = (
        f"H116 prospective, frozen spec "
        f"research/studies/overnight-range-mid-10d-drift-h116-prospective-spec.md. Single "
        f"pre-registered Validation-slice test of the Discovery-slice pass (hyp for "
        f"overnight_range_mid_10d_drift_h116, Discovery n=435 mean_r=0.682 ci_90=[0.506,0.854]). "
        f"Bucket edges frozen from Discovery, unmodified thresholds. Validation n={n}, "
        f"mean_r={r_result.get('mean_r_multiple_net')}, ci_90={r_result.get('ci_90')}. "
        f"Verdict: {verdict}. Full 90% promotion bar cleared: {full_promotion_bar_cleared}. "
        f"Full: data/study_overnight_range_mid_10d_drift_h116_prospective_results.json."
    )
    log_hypothesis(
        strategy_name="overnight_range_mid_10d_drift_h116_prospective",
