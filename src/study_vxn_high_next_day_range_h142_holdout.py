"""ONE-SHOT Holdout -- hyp-000142. Spec (frozen BEFORE this ran):
research/studies/vxn-high-next-day-range-h142-holdout-spec-frozen-2026-09-12.md.
= the Validation script (study_vxn_high_next_day_range_h142_validation.py) with
get_validation_data replaced by get_holdout_gen2_data and stage="holdout". Nothing
else changed: same edges, same bucket construction, same seed/N_BOOT, same P4 split.
HOW TO RUN: PYTHONPATH=src python3 src/study_vxn_high_next_day_range_h142_holdout.py
"""
import json, sys
...
from data_split import get_holdout_gen2_data
...
OUT = Path(__file__).resolve().parent.parent / "data" / "study_vxn_high_next_day_range_h142_holdout_results.json"
N_BOOT, SEED = 3000, 7
...
def main():
    df, syn = load_price_data(context="study_vxn_high_next_day_range_h142_holdout.py")
    if syn: raise SystemExit("ABORT synthetic")
    val = get_holdout_gen2_data(df)
    ...
    v = pwm.evaluate(label="hyp-000142_high_holdout", stage="holdout", n_obs=h["n"], mean=h["mean"], ci_90=tuple(h["ci_90"]), null_value=1.0, stage_trial_counts=tc)
    raw_pass = h["ci_90"][0] > 1.0 and (h["mean"] - 1.0) >= 0.05
    adj_pass = v.adjusted_ci[0] > 1.0 and (h["mean"] - 1.0) >= 0.05
    verdict = "HOLDOUT_PASS" if adj_pass else "HOLDOUT_FAIL"
    print(f"\nHIGH raw CI {h['ci_90']} raw_pass={raw_pass}; Sidak-adjusted (N_holdout={tc['holdout']}) CI {list(v.adjusted_ci)} adj_pass={adj_pass}\nVERDICT: {verdict}")
    ...
    OUT.write_text(json.dumps({"frozen_edges": FROZEN_EDGES, "n_days": int(len(f)), "cells": cells, "sidak": {"n_holdout_trials": tc["holdout"], "adjusted_ci": list(v.adjusted_ci)},
